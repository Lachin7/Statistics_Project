...
```
#### It seems like there is not much of a significant difference between districts. But we can see that the average price in dist 1, 2, 4, 6 might be a bit higher. 
#### let's also check this doing regression 
```{r}
...
#### Knowing that $ I = [\miu - 1.96 \sigma, \miu + 1.96 \sigma]$, we can see that  zero doesn't fall in to this interval and we can be sure that they have a linear relation. Meaning a house with more bedrooms, costs more (as expected).
